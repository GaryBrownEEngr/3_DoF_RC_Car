...
void print_crsf_packet(crsf_packet_t* p){
  Serial1.print("dest: 0x");
  Serial1.println(p->dest, 16);
  
  Serial1.print("len: 0x");
  Serial1.println(p->len, 16);

  Serial1.print("type: 0x");
  Serial1.println(p->type, 16);

  Serial1.print("crc: 0x");
  Serial1.println(p->crc, 16);

  Serial1.print("payload len: ");
  Serial1.println(p->payload_len);
  Serial1.print("payload:");
  for(uint8_t i = 0; i < p->payload_len; i++){
    Serial1.print(" 0x");
    Serial1.print(p->payload[i], 16);
  }
  Serial1.print("\n\n");
}
...
void process_crsf_packet(uint8_t rx_buffer[], uint8_t len){
  if( len < 4) {
    //Serial1.print("Message is too short\n");
    return;
  }
  crsf_packet_t p;
  p.dest = rx_buffer[0];
  ...
  switch(p.dest){
    ...
    case CRSF_ADDRESS_CRSF_RECEIVER:
      ...
      break;
    default:
      //Serial1.print("Unknown dest \n");
      return;
  }
  
  p.len = rx_buffer[1];
  if(p.len < 2) {
    //Serial1.print("given len is too small. \n");
    return;
  }
  ...
  if(p.packet_len > len) {
    //Serial1.print("message length is wrong. \n");
    return;
  }
  ...
  if(p.crc != computed_crc) {
    //Serial1.print("crc doesn't match computed crc: ");
    //Serial1.print(p.crc, 16);
    //Serial1.print(" ");
    //Serial1.println(computed_crc, 16);
    return;
  }
  ...
  if(p.type == CRSF_FRAMETYPE_RC_CHANNELS_PACKED){
    ...
    //Serial1.print("Packet type not expected: 0x");
    //Serial1.println(p.type, 16);
    //print_crsf_packet(&p);
  }
...
void process_crsf_channels_message(crsf_packet_t* p){
  ...
  /*
  Serial1.println(channel_commands.ch0);
  Serial1.println(channel_commands.ch1);
  Serial1.println(channel_commands.ch2);
  Serial1.println(channel_commands.ch3);
  Serial1.println(ch.ch4);
  Serial1.println(ch.ch5);
  Serial1.println(ch.ch6);
  Serial1.println(ch.ch7);
  */
  //Serial1.println(ch.ch8);
  //Serial1.println(ch.ch9);
  //Serial1.println(ch.ch10);
  //Serial1.println(ch.ch11);
  //Serial1.println(ch.ch12);
  //Serial1.println(ch.ch13);
  //Serial1.println(ch.ch14);
  //Serial1.println(ch.ch15);
}
